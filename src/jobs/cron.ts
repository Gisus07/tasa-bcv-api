 ...
 *  - Retry at `CRON_RETRY_AT` (default Mon-Fri 08:00 Caracas) that only
 *    runs if today's real (non-propagated) rate is still missing.
 *
 * Both are timezone-aware against America/Caracas; the host's TZ is irrelevant.
 */
export function startCron(): void {
  const e = env();
  const log = logger().child({ component: 'cron' });

  if (dailyTask || retryTask) {
  ...
  retryTask = cron.schedule(
    ...
    async () => {
      ...
      log.warn(
        { today, hasUsdReal: usdToday && !usdToday.isPropagated, hasEurReal: eurToday && !eurToday.isPropagated },
  ...
  log.info(
    { dailyAt: e.CRON_DAILY_AT, retryAt: e.CRON_RETRY_AT, tz: CARACAS_TZ },
    'cron scheduled',
  );
}
