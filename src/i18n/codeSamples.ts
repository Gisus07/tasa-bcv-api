...
function pythonSample(o: SampleOptions): string {
  ...
  if (o.bearer)
    lines.push(`headers = {"Authorization": f"Bearer {ADMIN_TOKEN}"}`);
...
function phpSample(o: SampleOptions): string {
  ...
  if (o.body) {
    lines.push(
      `curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(${phpRepr(o.body)}));`,
    );
  }
...
function goSample(o: SampleOptions): string {
  ...
  if (o.body) {
    lines.push(
      `  body := bytes.NewBufferString(${JSON.stringify(JSON.stringify(o.body))})`,
    );
    lines.push(
      `  req, _ := http.NewRequest("${method}", "${API_BASE}${o.path}", body)`,
    );
  } else {
    lines.push(
      `  req, _ := http.NewRequest("${method}", "${API_BASE}${o.path}", nil)`,
    );
  }
  if (o.bearer)
    lines.push(`  req.Header.Set("Authorization", "Bearer " + adminToken)`);
  if (o.body)
